...
public class TransferActivity extends AppCompatActivity {
    ...
    ArrayAdapter<String> myAdapterKodeBarang;
    List<String> listKodeBarang;
    ArrayList<String> listSpinner;

    //private String[] fruits = new String[]{"Apple", "Appy", "Banana", "Cherry", "Date", "Grape", "Kiwi", "Mango", "Pear"};
...
//        refresh = (ImageView) findViewById(R.id.nextbro);
                ...
                if (event.getAction() == KeyEvent.ACTION_DOWN) {
                    switch (keyCode) {
                        ...
                        case KeyEvent.KEYCODE_ENTER:
                            ...
                            if (et_code.length()<9) {
                                Toast.makeText(TransferActivity.this, "Masukkan Receipt Num Secara Lengkap!", Toast.LENGTH_LONG).show();
                                ...
                                adapter = new CursorAdapter(TransferActivity.this, helper.select());
                                hasildata.setAdapter(adapter);
...
//                        Toast.makeText(TransferActivity.this, "No Data", Toast.LENGTH_SHORT).show();
//                    } else {
...
//                    //checking
//                    if (mConn == null) {
//                        Toast.makeText(TransferActivity.this, "No Connection", Toast.LENGTH_SHORT).show();
//                    } else {
//
//                    }
//                    return true;
...
//        });
        ...
        search.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (et_code.length()<9) {
                    Toast.makeText(TransferActivity.this, "Masukkan Receipt Num Secara Benar!", Toast.LENGTH_LONG).show();
        ...
        hasildata.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                posisi = position;
                    ...
                    LoadingDialog();
                    System.out.println("Muncul Loading 1");
...
//        });
    ...
    void dialogInputQty() {
        ...
        final Spinner reason_code = (Spinner) viewInflate.findViewById(R.id.s_rcode);
        ArrayAdapter adapter = new ArrayAdapter(this,
                android.R.layout.simple_spinner_item, listSpinner);
        reason_code.setAdapter(adapter);
        ...
        final String segment = c.getString(c.getColumnIndexOrThrow("ITEM"));

        final String rcode = reason_code.getSelectedItem().toString();
...
//                        String newQty=tv_QtyReceive.getText().toString();
//                        String seq=segment;
//                        if (newQty.length()>0){
//                            helper.changeQty(newQty,seq);
//                            TransferActivity.refreshData();
//                            dialog.dismiss();
//                        }else {
//                            tv_QtyReceive.setError("Harus diisi!!!");
//                        }

                        if(getQC().equals("NON QC")){
                            Toast.makeText(TransferActivity.this, "Nomor Receipt Ini Tidak Butuh QC", Toast.LENGTH_LONG).show();
                        } else  {

                                if (et_qty.length() == 0) {
                                    Toast.makeText(TransferActivity.this, "Kosong", Toast.LENGTH_SHORT).show();
                                    ...
                                    if ( qtyTrans > qtyreceive) {
                                        Toast.makeText(TransferActivity.this, "Tidak boleh melebihi", Toast.LENGTH_SHORT).show();
                                        imm.toggleSoftInput(InputMethodManager.HIDE_IMPLICIT_ONLY, 0);
                                    } else {
                                        LoadingDialogAPIQC(et_code.getText().toString(), invIId(tv_code.getText().toString()), et_qty.getText().toString(), reason_code.getSelectedItem().toString());
    ...
    public String spinner() {
        ...
        try {
            ...
            System.out.println("SELECT mtr.REASON_NAME, mtr.DESCRIPTION FROM mtl_transaction_reasons mtr");
            theResultSet = statement.executeQuery("SELECT mtr.REASON_NAME, mtr.DESCRIPTION FROM mtl_transaction_reasons mtr");
...
//            listSpinner.add(0, "-Pilih Locator-");
    ...
    void LoadingDialog() {
        ...
        System.out.println("Muncul Loading 2");
    ...
    void LoadingDialogAPIQC(final String NomerReceipt, final String ItemId, final String Qty, final String ReasonCode) {
        LayoutInflater inflater = LayoutInflater.from(this);
        View viewInflated = inflater.inflate(R.layout.activity_loading, (ViewGroup) findViewById(android.R.id.content), false);
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                insertToMisc(NomerReceipt, ItemId, Qty,getPoNumber());
                commit();
                runApiQc(ReasonCode);
                commit();
                runApiDummy();
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(TransferActivity.this, "Proses Berhasil", Toast.LENGTH_SHORT).show();
                recreate();
            }
        }, 3000);
    }

    void runApiQc(String ReasonCode) {
        ...
        try {
            ...
            System.out.println("begin APPS.KHS_TRANSFER_PO('" + UserName + "','" + ReceiptNum + "','"+getPoNumber()+"','"+ReasonCode+"'); end;");

            //API issue
            CallableStatement funcnone = dbConnection
                    .prepareCall("begin APPS.KHS_TRANSFER_PO('" + UserName + "','" + ReceiptNum + "','"+getPoNumber()+"','"+ReasonCode+"'); end;");
    ...
    public void insertToMisc(String NomerReceipt, String ItemId, String Qty, String PoNumber) {
        Statement stmt = null;
        ...
        try {
            ...
            System.out
                    .println("insert into khs_delivery_po_temp (no_receipt,inventory_item_id,quantity,po_number) values ('" + ReceiptNum + "','" + ItemId + "','" + Qty + "','"+PoNumber+"')");

            stmt.executeUpdate("insert into khs_delivery_po_temp (no_receipt,inventory_item_id,quantity,po_number) values ('" + ReceiptNum + "','" + ItemId + "','" + Qty + "','"+PoNumber+"')");
    ...
    public void DeleteFromMisc(String NomerReceipt) {
        ...
        try {
            ...
            System.out
                    .println("DELETE FROM khs_delivery_po_temp WHERE no_receipt = '"+ReceiptNum+"'");

            stmt.executeUpdate("DELETE FROM khs_delivery_po_temp WHERE no_receipt = '"+ReceiptNum+"'");
    ...
    public boolean IsAny(String content) {
        ...
        try {
            ...
            Statement statement = mConn.createStatement();
            theResultSet = statement.executeQuery("SELECT \n" +
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_logout:
                new ManagerSessionUserOracle(TransferActivity.this).logoutUser();
                Intent i = new Intent(TransferActivity.this, LoginActivity.class);
